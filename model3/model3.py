            ...
            for epoch_i in range(epochs):
                ...
                if (epoch_i + 1) % display_step == 0:
                    ...
                    train_accuracy, train_loss = \
                        sess.run([model.accuracy, model.loss], feed_dict=feed)

                    validation_batch = mnist.validation.next_batch(_batch_size)

                    valid_feed = {
                        model.inputs: validation_batch[0],
                        model.targets: validation_batch[1],
                        model.keep_prob: 1.0,
                        model.batch_size: _batch_size
                    }

                    validation_accuracy, validation_loss = \
                        sess.run([model.accuracy, model.loss], feed_dict=valid_feed)
